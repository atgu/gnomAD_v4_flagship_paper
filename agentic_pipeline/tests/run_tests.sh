#!/usr/bin/env bash
# Test runner for the PEPPER / DisPo pipeline.
#
#   ./run_tests.sh            fast checks only (seconds)
#   ./run_tests.sh --full     + the DisPo regression (~4 min, 8 cores)
...
if [ "$run_full" -eq 1 ]; then
  note "Full regression (Monte Carlo recomputation, ~4 min)"
  ...
  note "Regression figure (4 scripts R, ~50 s)"
  python3 test_figure_regression.py || failures=$((failures+1))
else
